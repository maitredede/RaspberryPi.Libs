...
namespace RaspberryPi
{
    public sealed class Display : IDisposable
    {
        ...
        {
            var ret = Native.DispmanxNativeMethods.DisplayGetInfo(this.m_handle, out DISPMANX_MODEINFO_T infos);
            if (ret != DISPMANX_STATUS_T.SUCCESS)
                throw new DispmanException($"Operation failed : {nameof(Native.DispmanxNativeMethods.DisplayGetInfo)}");
            return infos;
        }
