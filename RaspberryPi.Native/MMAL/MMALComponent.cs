...
namespace RaspberryPi.MMAL
{
    ...
    public class MMALComponent : IDisposable
    {
        ...
        public void Acquire()
        {
            NativeMethods.ComponentAcquire(this.m_handle);
        }
        ...
        public void Release()
        {
            var status = NativeMethods.ComponentRelease(this.m_handle);
        ...
        public void Dispose()
        {
            NativeMethods.ComponentDestroy(this.m_handle);
        }
        ...
        public void Enable()
        {
            var status = NativeMethods.ComponentEnable(this.m_handle);
            if (status != MMAL_STATUS_T.MMAL_SUCCESS)
            {
                throw new MMALException(status);
            }
        ...
        public void Disable()
        {
            var status = NativeMethods.ComponentDisable(this.m_handle);
            if (status != MMAL_STATUS_T.MMAL_SUCCESS)
            {
                throw new MMALException(status);
            }
